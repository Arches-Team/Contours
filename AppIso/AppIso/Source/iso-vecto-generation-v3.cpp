...
/*
 * Les graphes R, Z et P ont la meme topology (celle donn�e par z)
 *
 * \param z Les diff�rentes zones, qu'importe leur valeur, elles seront tri�es de la plus petite � la plus grande, on va toujours commencer par la plus petite
 * \param p Les valeurs de proba qu'on veut donner. Elles doivent se situer dans l'intervalle [0, 1]
 * \param exactT Permet de dire si on veut suivre exactement T ou non (si oui on peut avoir des zones endor�iques)
 */
IsoVectoGenerationV3::IsoVectoGenerationV3(const GraphPoisson& z, const ScalarField2& p, bool withEndoreicZones) : R(z, INITIAL_VALUE), Z(z), P(z), TLow(z), THigh(z), T(z), withEndoreicZones(withEndoreicZones)
{
	...
	{
		// Pour �viter les erreurs avec des probas nulles
 ...
 *
 * \param debug Si != 0, nombre de debug � faire (un tous les `debug` frames) + les zones TLow, THigh et T
 * \param root Le dossier root + potentiel d�but de nom pour les images de debug
 */
...
void IsoVectoGenerationV3::CreateBorders()
{
	...
	// On assigne directement les �l�ments proche de la cote
	for (int nodeId = 0; nodeId < Z.Size(); ++nodeId)
	{
	...
	for (int nodeId = 0; nodeId < Z.Size(); ++nodeId)
	{
		// Un membre de la zone ne peut pas se situer sur une bordure d�s le d�but
		// Il est possible que le noeud soit de la zone, mais soit d�j� assign� (dans R donc) s'il �tait sur la bordure, donc on ne veut pas le r�assigner.
		...
		// Les bordures sont les �l�ments des zones externes qui sont au bord d'un �l�ment de la zone actuelle
		else
		{
...
void IsoVectoGenerationV3::DoubleEdenNodesAssignments()
{
	...
	// T repr�sente le pourcentage asc / (desc + asc) de l'eden
	// Plus on se rapproche de 1 et plus on est haut car proche de la barri�re haute (et inversement)
	// Dans certain cas (sommet de montagne), on n'a pas de valeur desc, dans ce cas, on consid�re �galement une valeur dans [0, 1] pour pouvoir trier.
	// Inversement dans certain cas (zone endor�ique), on n'a pas de valeur asc, on fait un calcul similaire pour avoir une valeur dans [0, 1].
	// Lorsqu'on est sur un sommet on donne la valeur (n �tant le nombre d'�l�ments dans la zone)
	//			asc / n
	// Cela permet aux sommets proches d'�tre directement choisi et de ne pas empi�ter sur ceux qui veulent aller haut
	// Avec le meme argument, pour les zones endor�ique on va donner
	//			(n - desc) / n
...
bool IsoVectoGenerationV3::DoubleEdenChooseNextNode(int valueToGive, bool internal)
{
	...
	while (true)
	{
		// On r�cup�re un �l�ment de bordure (on inverse les proba si on vient de l'int�rieur (phase descendante)
		int parent = GetRandomNode(borders, internal);

		// Certaines zones n'ont pas de bordures internes (isos de pics)
		// Certaines zones n'ont pas de bordures externes (isos endor�ique)
		...
		// On choisit un des voisins de mani�re al�atoire
		QVector<int> neighs;
		...
		// On enl�ve un noeud de l'ensemble des bordures que lorsqu'on le choisi al�atoirement et qu'on se rend compte que tous les voisins sont choisis
		// TODO: c'est sans doute plus rapide de garder le set le plus petit possible � chaque fois, mais c'est plus clair et plus rapide � coder comme �a
		if (!neighs.empty())
		{
...
void IsoVectoGenerationV3::FinalNodesAssignments()
{
	...
	// On assigne selon l'ordre de T, meme si cela fait des zones endor�iques
	...
	// On assigne selon l'ordre de T, mais seulement en fonction des �l�ments de T proche de la bordure actuelle
	else
	{
		for (int i = 0; i < sizeZone; ++i)
		{
...
bool IsoVectoGenerationV3::FinalChooseNextNode()
{
	...
	// On r�cup�re l'�l�ment de la bordure � valeur la plus basse (PAS DE PROBA)
	...
	// Tous les voisins non assign�s deviennent des �l�ments accessibles
...
// Renvoie un noeud al�atoire, en fonction des bordures donn�es. On inverse lorsqu'on part de la bordure externe pour que les probas signifient la meme chose
int IsoVectoGenerationV3::GetRandomNode(const QSet<int>& borders, bool inverseProba)
{
	...
	for (int ind : borders)
	{
		// TODO: normalement les probas vont de 0 � 1, donc c'est plus simple de faire �a pour inverser les probas, mais ptetre faire diff�remment
		if (inverseProba) cumul += 1 - P[ind];
		else			  cumul += P[ind];
	}

	// Si aucun point ne peut �tre choisi, on n'en choisi pas
	if (cumul == 0)
		return -1;
