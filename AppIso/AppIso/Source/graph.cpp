...
/*
 * Construit un graphe � partir d'un sample de poisson � l'int�rieur d'un polygone
 * 
 * \param mask Le masque d�finissant quelle zone est dans ou hors du graphe (doit �tre un masque binaire 0 = hors de la zone)
...
{
	// On scale un peu pour �tre plus gros que le masque et �viter les voisins moches � cause de la Delaunaysation
	Box2 b = mask.GetBox();
	Vector2 border(8*r, 8*r);
	b = Box2(b[0] - border, b[1] + border);

	Mesh2 mesh = Misc::DelaunayPointsInBox(b, r);
	Mesh2 mesh2 = mesh.SubMesh(mask); // TODO: l�ger bug dans cette fonction, certains points en bordure de mer n'auront pas de voisins
...
/*
 * Initialise les valeurs du graphe avec le bruit donn� 
 */
 ...
 * 
 * TODO: Pour le moment cette m�thode supprime la valeur 0 car on l'utilise de cette mani�re
 */
void GraphPoisson::SetStrictValueFromScalarField(const ScalarField2& sf)
{
	...
	// On ne veut pas de 0, donc on r�cup�re les valeurs des voisins
	while (!zeroes.empty())
	{
		QSet<int> newZeroes;
		for (int id : zeroes)
		{
			// On r�cup�re la plus petite valeur voisine != 0
			double v = 0.0;
		...
		// On abandonne si la zone est une petite zone seule pour �viter la boucle infinie
...
void GraphPoisson::SetValueFromHistogram(const HistogramD& histo)
{
	...
	// TODO: ne pas passer par l�, o� alors faire en sorte que le ToHistogram soit plus coh�rent, voir ce qui �tait fait avant dans GetNextHeight (ancien commit)
...
/*
 * Renvoi un champs scalaire dans la range donn�e correspondant aux valeurs interpol�es du maillage
 */
...
Polygons2 GraphPoisson::ContourLines(double h) const
{
	...
	double exteriorValue = seaValue - 100 * (max - min + 1); // valeurs tr�s basse

	// On donne les valeurs � topologyExt, on fait le marching triangle sur ce graphe l�
...
// Renvoie vraie si l'indice de la topologie ext�rieur correspond � un sommet dans la box du masque o� � l'ext�rieur
