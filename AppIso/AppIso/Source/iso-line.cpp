...
void IsoLinePoly::Epurate(double eps)
{
	...
	// On v�rifie la relation entre le premier et le dernier point
	// On continue tant que le dernier segment est trop gros (car rien n'affirme qu'il ne sera pas trop grand une fois le dernier point supprim�)
...
IsoLinePoly IsoLinePoly::Resample(double distMax) const
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		// ajout points interm�diaires si besoin
		if (dist > distMax)
		{
...
{
	int n = Size();
	// On fait un set pour �viter de devoir v�rifier tous les indices
...
\todo Pass this function into Polygon2 class
	  ...
	  Peut �tre am�lior� avec l'algorithme de Bentley-Ottmann en O((n + m)log(n + m))
...
IsoLinePoly::IntersectType IsoLinePoly::RelationWith(const Polygon2& p) const
{
	...
	// Si on n'a aucune intersection avec un segment, on a trois possibilit�s
	// 1. On contient compl�tement l'autre polygone
	if (Inside(p.Vertex(0)))
	{
		return IntersectType::CONTAINS;
	}

	// 2. On est compl�tement contenu dans l'autre polygone
	if (p.Inside(q.at(0)))
	{
		return IntersectType::INSIDE;
	}

	// 3.Les deux polygones sont compl�tement disjoints
...
Vector2 IsoLinePoly::VertexNormal(int i) const
{
	...
	// Les points sont align�s, donc la bissectrice est ind�finie
	// mais dans notre cas, on veut juste le vecteur � 90 deg CCW (trigonometric order) par rapport au Vector2 v1
	if (Norm(bisector) < 1e-12)
	{
		return Vector2(-v1[1], v1[0]);
	}

	// La bissectrice pointe toujours vers l'int�rieur de l'angle
	// Donc si on a un angle obtu (produit vectoriel n�gatif) on envoie l'inverse
	if (v1 / v2 < 0)
...
{
	// Cas sp�cifique qui fait buguer `LineSegments()`
	if (sf.VertexSize() == 0)
		return;
	...
	// Le but de tout ce calcul est d'�viter d'avoir une hauteur qui soit exactement une valeur d'un pixel du champ scalaire.
	// Si c'est le cas, on risque d'avoir des segments qui se chevauchent et qui sont consid�r�s comme s'auto-croisant, � cause des "cr�tes".
	// Ces cr�tes se forment lorsque 2 pixels voisins de meme valeur sont entour�s par des valeurs + grandes ou + basses.
	// Donc on fait en sorte qu'aucune hauteur ne soit exactement la valeur d'un pixel pour �viter que �a arrive.
	...
	for (double& h : values)
		if (specificHeights.contains(h))
			h = h + (max - min) / (heights.size() * 100); // Attention, petit epsilon qui ne marche pas n�cessairement avec tous les terrains, mais suffisant pour l'instant
	// Fin correction
	...
	for (int i = 0; i < n; ++i)
	{
		...
		for (int j = 0; j < p2.Size(); ++j)
		{
			...
			ilp.Epurate(0.001);
			// On �tend le long de la normal en fonction de la hauteur pour ne pas avoir de polygone ayant une ar�te en commun, ce qui est souvent le cas si on n'est pas sur une ile. Et les tests avec des polygones qui partagent un segment c'est relou.
...
// R�cup�re les isos entre les valeurs de histo (utilis� pour donner les nodes du graphe poisson)
// Utilise le fait que le graph poisson a �t� assign� via une classe IsoVectoGeneration
// On r�cup�re les isos au hauteur (h[i] + h[i+1]) / 2 pour �viter les iso "cr�tes" (voir article)
...
QGraphicsScene* IsoLines::ToScene(const DisplayOptions& opt) const
{
	...
	std::function<void(int, int)> draw = [&](int i, int level)
	{
		...
		if (should_display)
		{
			...
			pen.setWidth(l);
			// TODO: c'est bien pour zoomer, mais �a marche pas bien sur une sc�ne a exporter
