...
{	
	...
	fragVertex = geomVertex[0]; fragNormal = geomNormal[0]; fragColor = geomColor[0]; fragUV = geomUV[0];
	EmitVertex();

	dist = vec3(0, area / length(v1), 0);
	gl_Position = gl_in[1].gl_Position;
	fragVertex = geomVertex[1]; fragNormal = geomNormal[1]; fragColor = geomColor[1]; fragUV = geomUV[1];
	EmitVertex();

	dist = vec3(0, 0, area / length(v2));
	gl_Position = gl_in[2].gl_Position;
	fragVertex = geomVertex[2]; fragNormal = geomNormal[2]; fragColor = geomColor[2]; fragUV = geomUV[2];
	EmitVertex();
...
vec4 GetBaseColor()
{
	...
	// Texture
	if (material == 3 && hasTexture == 1)
	{
		vec4 texColor = texture(diffuseTex, fragUV);
		return vec4(texColor.rgb * Diffuse(fragNormal, normalize(-viewDir)), texColor.a);
	}
}
