...
layout(location = 32) uniform int show_sides = 1;
layout(location = 33) uniform int show_render_camera = 0;
layout(location = 34) uniform ivec2 render_res = ivec2(2560, 1440) ;
	...
	*/
	...
	if (hit)
	{
		...
		if (abs(Box(p.xy, a, b)) < epsilon || abs(p.z - zRange.x + 0.1f * (zRange.y - zRange.x)) < epsilon) {
			float bdx = (Box(p.xy + vec2(0.1, 0.), a, b) - Box(p.xy + vec2(-0.1, 0.), a, b)) / 0.2;
			float bdy = (Box(p.xy + vec2(0., 0.1), a, b) - Box(p.xy + vec2(0., -0.1), a, b)) / 0.2;
			vec3 boxn = normalize(vec3(bdx, bdy, 0.));
			float alpha = abs(dot(boxn, -normalize(lightDir)));
			c = vec4(vec3(0.2 + 0.2 * alpha), 1.);
	...
	// check if out of render camera
	if (show_render_camera != 0) {
		float ratio = min(iResolution.x / float(render_res.x), iResolution.y / float(render_res.y));
		vec2 corner_a = 0.5 * iResolution - 0.5 * ratio * vec2(render_res.x, render_res.y);
		vec2 corner_b = 0.5 * iResolution + 0.5 * ratio * vec2(render_res.x, render_res.y);
		if (gl_FragCoord.x < corner_a.x || gl_FragCoord.x > corner_b.x || gl_FragCoord.y < corner_a.y || gl_FragCoord.y > corner_b.y) {
			c.rgb *= 0.3;
			//c.a = 1.0;
			gl_FragDepth = 0.;
		}
