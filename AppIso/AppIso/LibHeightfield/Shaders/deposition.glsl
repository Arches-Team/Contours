...
float cellArea = (b.x - a.x) * (b.y - a.y) / float((nx - 1) * (ny - 1)) * 0.00001;
...
void main() {
	...
	//float speed = steepest_slope * steepest_slope;
    float streamPower = pow(stream, 0.3) * speed;
    out_debug[id] = 0.;

	// Deposit
    //float deposition_strength = 3.;// (y > ny / 2) ? 1. : 0.;
	...
	// Erode
		...
		//sed += (deposition_strength * streamPower - sed) * 0.1;
		//sed += 0.5 * deposition_strength * streamPower;

        /*float add_sed = abs(steepest_slope);
        add_sed = (add_sed > 0.5)?add_sed:0.;
        add_sed = min(add_sed, 2.);

        out_debug[id] = add_sed;*/
