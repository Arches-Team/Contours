...
// delta h
layout(binding = 4, std430) readonly buffer InDelta { float in_delta[]; };
layout(binding = 5, std430) writeonly buffer OutDelta { float out_delta[]; };

// hardness
layout(binding = 6, std430) readonly buffer Hardness { float in_hardness[]; };
...
void main() {
    ...
    // do not erode borders
    if (p.x == 0 || p.x == nx - 1 || p.y == 0 || p.y == ny - 1) {
        out_terrain[id] = in_terrain[id];
    ...
    spe *= k;
    //float spe = k * pow(stream, p_sa) * pow(steepest_slope, p_sl);
    ...
    out_delta[id] = new_height - old_height;
    out_stream[id] = stream;

    /*float spe = k * pow(in_delta[id], p_sa) * clamp(pow(steepest_slope, p_sl), 0., 1.);

    float old_height = in_terrain[id];
    float new_height = old_height;
    new_height -= dt * spe * in_hardness[id];
    new_height = max(new_height, receiver_height);
